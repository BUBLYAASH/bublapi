...
function showPatientBookingStep(step) {
    ...
    document.querySelector('#view-patient-appointments .booking-stepper')?.scrollIntoView({
        behavior: 'smooth',
        block: 'start'
    });
}
...
function canAdvancePatientBooking(nextStep) {
    if (nextStep >= 2 && !$('#patientServiceSelect')?.value) {
        toast('Сначала выберите услугу', 'error');
        return false;
    }
    if (nextStep >= 3 && !$('#patientDoctorSelect')?.value) {
        toast('Выберите специалиста', 'error');
        return false;
    }
...
function viewFromPathname() {
    ...
    return ROUTE_VIEWS[path] || document.querySelector('[data-initial-view]')?.dataset.initialView || 'home';
}
...
function resolveOpaqueRef(key) {
    return opaqueRefs.get(key);
}

function resetOpaqueRefsByPrefix(prefix) {
    for (const key of [...opaqueRefs.keys()]) if (key.startsWith(`${prefix}_`)) opaqueRefs.delete(key);
}
...
function showView(name, {updateUrl = true, load = true} = {}) {
    ...
    const commitView = () => {
        ...
        if (updateUrl) {
            const targetPath = VIEW_ROUTES[name] || '/';
            if (location.pathname !== targetPath) history.pushState({view: name}, '', targetPath);
        }
    };
...
window.addEventListener('popstate', () => {
    showView(viewFromPathname(), {updateUrl: false});
});
...
async function checkConnection() {
    try {
        ...
        const badge = $('#connectionStatus');
        if (badge) {
            badge.textContent = 'Сервис временно недоступен';
...
const APPOINTMENT_STATUS_SINGLE = {
    CREATED: 'Создана',
...
function appointmentStatusLabel(status) {
    return APPOINTMENT_STATUS_SINGLE[String(status || '').toUpperCase()] || String(status || '—');
}
...
async function loadPublicDoctors({silent = false} = {}) {
    try {
        ...
        renderCards(
            ...
            doctorsWithHours,
            doctor => {
                const ref = makeOpaqueRef('publicDoctor', doctor.id);
                return `
        ...
        renderServiceStatusGroups(
            ...
            (container, groupServices) => {
                ...
                renderCards(
                    ...
                    groupServices,
                    service => {
                        const ref = makeOpaqueRef('publicService', service.id);
                        return `
...
async function loadPatientAppointments({silent = false} = {}) {
    try {
        ...
        renderAppointmentGroups(
            ...
            [
                {
                    label: 'Дата',
                    render: appointment => {
                        const ref = makeOpaqueRef('patientAppointment', appointment.id);
                        return `
            ...
            appointment => {
                const ref = makeOpaqueRef('patientAppointment', appointment.id);
                return String(appointment.status || '').toUpperCase() === 'CANCELLED'
                    ? ''
                    : `
...
async function loadNotifications({silent = false} = {}) {
    try {
        ...
        renderCards(
            ...
            notifications,
            notification => {
                const ref = makeOpaqueRef('notification', notification.id);
                return `
...
function renderStaffAttention(appointments, patients) {
    ...
    const items = [];
    if (created.length) items.push({
        tone: 'warning',
        icon: '◷',
        title: 'Все неподтверждённые записи',
        value: created.length,
        text: 'Ожидают подтверждения сотрудником клиники — независимо от даты приёма.',
        view: 'staff-appointments'
    });
    if (confirmed.length) items.push({
        tone: 'info',
        icon: '◎',
        title: 'Подтверждены на сегодня',
        value: confirmed.length,
        text: 'Проверьте готовность кабинетов и специалистов.',
        view: 'staff-appointments'
    });
    if (cancelled.length) items.push({
        tone: 'danger',
        icon: '×',
        title: 'Отмены сегодня',
        value: cancelled.length,
        text: 'Можно использовать освободившиеся окна для новых записей.',
        view: 'staff-appointments'
    });
    ...
    if (upcoming.length) items.unshift({
        tone: 'success',
        icon: '→',
        title: 'Приёмы в ближайший час',
        value: upcoming.length,
        text: `Ближайший: ${staffAppointmentPatientName(upcoming[0], patientMap)}.`,
        view: 'staff-appointments'
    });
...
$('#view-staff-dashboard')?.addEventListener('click', async event => {
    ...
    if (createButton) {
        await openStaffWorkflow('create-appointment', {
            doctorId: createButton.dataset.doctorId,
            date: createButton.dataset.date
        });
        return;
    }
    ...
    if (event.target.closest('#staffCalendarPrev')) {
        shiftStaffCalendarDate(-1);
        return;
    }
    if (event.target.closest('#staffCalendarNext')) {
        shiftStaffCalendarDate(1);
        return;
    }
    if (event.target.closest('#staffCalendarToday')) {
        setStaffCalendarDate(dashboardDateKey(new Date()));
        return;
    }

    if (appointmentButton) {
        try {
            await getStaffAppointmentById(appointmentButton.dataset.calendarAppointment);
        } catch (error) {
            toast(error.message, 'error');
        }
    }
...
$('#staffDoctors').addEventListener('click', async event => {
    ...
    try {
        if (book) {
            await openStaffWorkflow('create-appointment', {doctorId: book.dataset.id});
            return;
        }
        if (scheduleAction) {
            await openStaffWorkflow('doctor-schedule', {doctorId: scheduleAction.dataset.id});
            return;
        }
        if (details) {
        ...
        [
          ...
          `
            },
            {
                label: 'Пациент',
                render: appointment => {
                    const patient = caches.patients.get(String(appointment.patientId)) || caches.patients.get(appointment.patientId);
                    return escapeHtml(patient ? [patient.lastName, patient.firstName].filter(Boolean).join(' ') : appointment.patientId);
                }
            },
            {
                label: 'Врач',
...
$('#detailModalActions').addEventListener('click', async event => {
    try {
        ...
        if (appointmentDoctor?.dataset.doctorId) {
            await getStaffDoctorById(appointmentDoctor.dataset.doctorId);
            return;
        }
        ...
        if (patientBook) {
            closeModal();
            await openStaffWorkflow('create-appointment', {patientId: patientBook.dataset.id});
            return;
        }
        if (doctorBook) {
            closeModal();
            await openStaffWorkflow('create-appointment', {doctorId: doctorBook.dataset.id});
            return;
        }
        if (doctorSchedule) {
            closeModal();
            await openStaffWorkflow('doctor-schedule', {doctorId: doctorSchedule.dataset.id});
            return;
        }
        if (doctorServices) {
            closeModal();
            await openStaffWorkflow('doctor-services', {doctorId: doctorServices.dataset.id});
            return;
        }
        if (serviceBook) {
            closeModal();
            await openStaffWorkflow('create-appointment', {serviceId: serviceBook.dataset.id});
            return;
        }
        if (userBook) {
            ...
            if (!patient) {
                toast('У пользователя пока нет карточки пациента', 'error');
                return;
            }
            closeModal();
            await openStaffWorkflow('create-appointment', {patientId: patient.id});
            return;
        }
...
['staffPatients', 'staffDoctors', 'staffServices', 'staffAppointments', 'staffUsers'].forEach(id => {
    const el = document.getElementById(id);
    if (el) staffListObserver.observe(el, {childList: true, subtree: true});
});
